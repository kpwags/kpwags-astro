...
export const getPostsPerYear = (posts: BlogPost[]): PostsPerYear[] => {
    ...
    for (let i = 0; i < years.length; i += 1) {
        postsPerYear.push({
            year: years[i],
            count: posts.filter((p) => getPostYear(p.date.toString()) === years[i]).length,4
        });
    }
...
While working on the years, I realized that when I converted my site to run on [Next.js](https://nextjs.org), I added tags to all of my posts. While there are many tags I’ve used, I figured the ten most popular could be a good item to show.
...
This is an example from my [Reading Log - January 20 (#15)](https://kpwags.com/reading-log/15) post. Each tag has 2 properties. The display name, and the URL slug that corresponds to it. I’ve toyed with trying to auto-generate it, but this has worked for me thus far.
...
![The end result of my posts per year chart showing horizontal bars representing the number of posts per year](../../assets/images/posts/2023-02-03-building-a-stats-page/posts-per-year.png)
...
![The end result of my most popular tags chart displaying horizontal bars representing posts per tag](../../assets/images/posts/2023-02-03-building-a-stats-page/most-popular-tags.png)
